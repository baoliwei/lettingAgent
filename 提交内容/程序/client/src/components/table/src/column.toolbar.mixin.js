  ...
  methods: {
    ...
    renderColumnToolbar (h, item = {}) {
      ...
      const props = {
        ...
        width: item.width || 46,
        'render-header' (h, scope) { // eslint-disable-line
          return (
            <el-button
              onClick={self.onColumnToolbar.bind(self)}
              size="mini"
              class="rc-table__column-toolbar"
              type="text"
        ...
        render: (h) => {
          return (
            <el-transfer
              ...
              titles={['隐藏字段', '显示字段']}
              onInput={val => this.sync('currentVisibleColumnToolbarValue', val)}
              props={props}
              data={editColumns}>
            </el-transfer>
          )
