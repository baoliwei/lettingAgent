...
export default {
  ...
  methods: {
    ...
    getColumnRenderKey (index) {
      return `__rc-table-${uuid}_column__${index}`
    },
    // 页面改变
  ...
  render (h) {
    ...
    return (
      <div class="rc-table" padding>
        {
          this.$slots.toolbar && this.$slots.toolbar
        }
        <el-table ref="table" key={`'__rc-table-uu${uuid}'`} data={this.list} style="width: 100%;" size="small" on-sort-change={this.sortChange} {...data}>
          {
